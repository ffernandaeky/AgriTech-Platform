...
// Import icons sesuai path project kamu
import { 
  CalenderIcon, 
  EyeIcon, 
  UserIcon,
...
export default function BudidayaOrganikPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const res = await fetch("/api/videos/budidaya-organik");
        ...
        console.error("Gagal memuat data budidaya organik:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return (
    <div className="p-10 text-center flex flex-col items-center justify-center min-h-[400px]">
      ...
      <p className="text-gray-500 font-medium font-sans uppercase tracking-widest text-xs">Menyinkronkan Materi Budidaya Organik...</p>
    </div>
  );

  return (
    // PERBAIKAN: Background abu-abu dihapus (bg-transparent) agar estetik dan ringan
      ...
      <section>
        <div className="flex items-center gap-3 mb-8">
          ...
          <h2 className="text-xl font-bold uppercase tracking-tight text-gray-800 dark:text-white">
            video paling banyak ditonton pada topik budidaya organik
          </h2>
        </div>
      ...
      <section>
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <div className="h-8 w-1.5 bg-brand-500 rounded-full"></div>
            <h2 className="text-xl font-bold uppercase tracking-tight text-gray-800 dark:text-white">
              video lainnya pada topik budidaya organik
            </h2>
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div className="px-10 py-12">
              <div className="mb-4">
                <span className="px-3 py-1 bg-green-100 text-green-700 dark:bg-green-500/10 dark:text-green-400 rounded-lg text-[10px] font-black uppercase tracking-widest border border-green-200 dark:border-green-500/20">
                  {selectedVideo.pred_label || "Budidaya Organik"}
                </span>
              </div>

              <h2 className="text-xl font-extrabold text-gray-900 dark:text-white mb-6 leading-tight">
                {selectedVideo.title}
              </h2>
              ...
              <div className="flex flex-wrap gap-5 mb-8 py-5 border-y border-gray-100 dark:border-white/[0.05] bg-gray-50 dark:bg-white/[0.02] rounded-2xl px-5">
                <div className="flex items-center gap-2 text-xs font-bold text-gray-700 dark:text-gray-300 italic">
                  <UserIcon className="size-4 text-brand-600" /> {selectedVideo.channel_name}
                </div>
                <div className="flex items-center gap-2 text-xs font-bold text-gray-700 dark:text-gray-300 italic">
                ...
                <div className="flex items-center gap-2 text-xs font-bold text-gray-700 dark:text-gray-300 italic">
                  <EyeIcon className="size-4 text-blue-600" /> {Number(selectedVideo.view_count).toLocaleString('id-ID')} Views
                </div>
                <div className="flex items-center gap-2 text-xs font-bold text-gray-700 dark:text-gray-300 italic">
              ...
              <div className="max-h-[250px] overflow-y-auto custom-scrollbar pr-4">
                ...
                <p className="text-[13px] text-gray-600 dark:text-gray-400 leading-relaxed whitespace-pre-line font-medium italic">
                  {selectedVideo.description || "Video edukasi mengenai teknik budidaya organik..."}
                </p>
              </div>

              <div className="mt-8 flex justify-end gap-3 border-t border-gray-100 dark:border-gray-800 pt-6">
                <button onClick={() => setSelectedVideo(null)} className="px-8 py-3 bg-gray-100 dark:bg-white/[0.05] text-gray-700 dark:text-gray-300 font-bold text-xs rounded-2xl hover:bg-gray-200 dark:hover:bg-white/[0.1] transition-all uppercase tracking-widest">Tutup</button>
                <a href={selectedVideo.video_url} target="_blank" rel="noopener noreferrer" className="px-8 py-3 bg-green-600 text-white font-bold text-xs rounded-2xl hover:bg-green-700 transition-all uppercase tracking-widest shadow-lg shadow-green-600/20">Lihat Video</a>
              </div>
            </div>
          </div>
        </div>,
  ...
  return (
    <div className="group flex flex-col h-full rounded-[2.5rem] border border-gray-100 bg-white p-8 shadow-sm transition-all duration-300 hover:shadow-xl hover:-translate-y-1 dark:border-gray-800 dark:bg-white/[0.03]">
      ...
      <p className="text-sm text-gray-500 dark:text-gray-400 line-clamp-3 mb-8 leading-relaxed italic">
        {'"'}{video.description || "Materi edukasi mengenai teknik budidaya organik..."}{'"'}
      </p>

      <div className="grid grid-cols-2 gap-y-4 gap-x-2 mb-8 mt-auto pt-6 border-t border-gray-50 dark:border-gray-800">
        <div className="flex items-center gap-2 text-[11px] font-bold text-gray-600 dark:text-gray-400 italic">
      ...
      <div className="flex gap-3">
        <button onClick={onDetail} className="flex-1 px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white text-[11px] font-black rounded-full shadow-lg transition-all active:scale-95 uppercase tracking-widest text-center">Detail Video</button>
        <a href={video.video_url} target="_blank" rel="noopener noreferrer" className="flex-1 px-4 py-3 bg-[#006622] hover:bg-[#004d1a] text-white text-[11px] font-black rounded-full shadow-lg transition-all active:scale-95 uppercase tracking-widest text-center">Lihat Video</a>
      </div>
    </div>
  );
}

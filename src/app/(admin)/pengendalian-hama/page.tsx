...
export default function PengendalianHamaPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const res = await fetch("/api/videos/pengendalian-hama");
  ...
  return (
    // PERBAIKAN: bg-gray-50/30 dihapus dan diganti menjadi bg-transparent agar menyatu dengan dashboard[cite: 1]
    <div className="space-y-12 p-4 md:p-8 bg-transparent min-h-screen relative font-sans transition-all duration-300">
      ...
      <section>
        <div className="flex items-center gap-3 mb-8">
          ...
          <h2 className="text-xl font-bold uppercase tracking-tight text-gray-800 dark:text-white">
            Video Paling Banyak Ditonton Pada Topic Pengendalian Hama
          </h2>
        </div>
      ...
      <section>
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            <div className="h-8 w-1.5 bg-brand-500 rounded-full"></div>
            <h2 className="text-xl font-bold uppercase tracking-tight text-gray-800 dark:text-white">
              Video Lainya Pada Topic Pengendalian Hama
            </h2>
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      ...
      {/* --- MODAL DETAIL (FIXED BLUR ALL SCREEN) --- */}
      {mounted && selectedVideo && typeof document !== 'undefined' && createPortal(
        <div className="fixed inset-0 z-[999999] flex items-center justify-center p-4 bg-black/40 backdrop-blur-md transition-all animate-in fade-in duration-300">
          <div className="bg-white dark:bg-gray-900 w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in duration-300 font-sans relative">
            
            <div className="absolute top-4 right-4">
              <button onClick={() => setSelectedVideo(null)} className="p-2 text-gray-400 hover:text-gray-600 dark:hover:text-white transition-colors">
            ...
            <div className="px-8 py-10">
              <div className="mb-4">
                <span className="px-3 py-1 bg-green-100 text-green-700 rounded-lg text-[10px] font-black uppercase tracking-widest border border-green-200">
                  {selectedVideo.pred_label || "Pengendalian Hama"}
                </span>
              </div>
              ...
              <div className="flex flex-wrap gap-5 mb-8 py-4 border-y border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-white/5 rounded-xl px-4">
                <div className="flex items-center gap-2 text-xs font-bold text-gray-700 dark:text-gray-300">
                  <UserIcon className="size-4 text-brand-600" /> {selectedVideo.channel_name}
                </div>
                <div className="flex items-center gap-2 text-xs font-bold text-gray-700 dark:text-gray-300">
                  <CalenderIcon className="size-4 text-orange-600" /> 
              ...
              <div className="max-h-[250px] overflow-y-auto custom-scrollbar pr-2 mb-2">
                <h4 className="text-[10px] font-black uppercase text-gray-400 mb-3 tracking-widest">Isi Deskripsi Lengkap:</h4>
                <p className="text-[13px] text-gray-600 dark:text-gray-400 leading-relaxed whitespace-pre-line font-medium italic">
                  {selectedVideo.description || "Video edukasi mengenai teknik pengendalian hama tanaman..."}
                </p>
              </div>

              <div className="mt-8 flex justify-end gap-3 border-t border-gray-100 dark:border-gray-800 pt-6">
                <button onClick={() => setSelectedVideo(null)} className="px-8 py-3 bg-gray-100 text-gray-700 font-bold text-sm rounded-2xl hover:bg-gray-200 uppercase transition-all">Tutup</button>
                <a href={selectedVideo.video_url} target="_blank" rel="noopener noreferrer" className="px-8 py-3 bg-[#006622] text-white font-bold text-sm rounded-2xl hover:bg-[#004d1a] uppercase shadow-lg">Lihat Video</a>
              </div>
            </div>
          </div>
        </div>,
  ...
  return (
    <div className="group flex flex-col h-full rounded-[2.5rem] border border-gray-100 bg-white p-8 shadow-sm transition-all duration-300 hover:shadow-xl hover:-translate-y-1 dark:border-gray-800 dark:bg-white/[0.03]">
      ...
      <p className="text-sm text-gray-500 dark:text-gray-400 line-clamp-3 mb-8 leading-relaxed italic">
        {'"'}{video.description || "Materi edukasi mengenai teknik pengendalian hama..."}{'"'}
      </p>

      {/* METADATA: GRID 2 KOLOM */}
